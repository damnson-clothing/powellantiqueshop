<template>
  <NuxtLayout name="admin">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Error Alert -->
      <div v-else-if="error" class="mb-6 bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
        {{ error }}
      </div>

      <!-- Product Form -->
      <form v-else @submit.prevent="handleSubmit" class="space-y-8 bg-white shadow sm:rounded-lg p-6">
        <!-- Basic Information -->
        <div class="space-y-6">
...
const fetchProduct = async () => {
  try {
    ...
    const product = await $fetch<any>(`/api/admin/products/${productId}`)
